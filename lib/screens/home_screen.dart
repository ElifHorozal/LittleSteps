...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _addHabit(
      String habitName, String chartType, Color customColor) async {
  ...
  Future<void> _checkAndResetWeeklyProgress(String habitId) async {
    ...
    if (user != null) {
      final today = DateTime.now();
      final weekStart =
          getStartOfWeek(today); // Haftanın Pazartesi'sini hesapla

      final normalizedWeekStart =
          DateTime(weekStart.year, weekStart.month, weekStart.day); // Normalize
      ...
      if (habitSnapshot.exists) {
        ...
        if (normalizedWeekStart.isAfter(normalizedCurrentWeekStart)) {
          ...
          await habitRef.update({
            'weeklyProgress': List<bool>.generate(
                7, (_) => false), // Haftalık ilerleme sıfırlanır
            'currentWeekStart': DateFormat('yyyy-MM-dd')
                .format(normalizedWeekStart), // Yeni hafta başlangıç tarihi
          });
        } else {
                              ...
                              children: [
                                ...
                                Checkbox(
                                  ...
                                  onChanged: (bool? value) async {
                                    if (value != null) {
                                      await _habitHelper.updateDailyProgress(
                                          habit.id, value);
                                    }
                                  },
                                ),
